...
@app.post(
    ...
    response_model=dict[str, Any],
    responses={
        200: {'description': 'Dataset uploaded successfully', 'model': dict},
        400: {'description': 'Invalid destination path', 'model': dict},
        500: {'description': 'Error uploading dataset', 'model': dict},
    },
)
async def upload_dataset(
    ...
    """Upload a dataset file to the specified destination in the workspace.

    This endpoint allows uploading dataset files (CSV, JSON, Excel, etc.) to be used
    in ML pipelines. The files are saved to the specified destination in the workspace.

    Args:
        request (Request): The incoming request object.
        file (UploadFile): The file to upload.
        destination (str): The destination directory in the workspace.
    ...
    try:
        ...
        full_dest_path = os.path.join(runtime.config.workspace_mount_path_in_sandbox, destination.lstrip('/'))
        ...
        logger.info(f'Dataset file {file.filename} uploaded to {destination}')
        ...
        return JSONResponse(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            content={'error': f'Error uploading dataset file: {e}'},
        )
