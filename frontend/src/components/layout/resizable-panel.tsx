...
export function ResizablePanel({
  ...
  return (
    <div className={twMerge("flex", !isHorizontal && "flex-col", className)}>
      <div
        ref={firstRef}
        className={twMerge(firstClassName, "transition-all duration-300 ease-in-out")}
      ...
      <div
        className={`${
          isHorizontal ? "cursor-ew-resize w-3 flex-col" : "cursor-ns-resize h-3 flex-row"
        } shrink-0 flex justify-center items-center bg-gradient-to-r from-base-secondary to-base relative`}
        onMouseDown={collapse === Collapse.SPLIT ? onMouseDown : undefined}
      >
        <div className="absolute inset-0 flex items-center justify-center">
          <div className={`${isHorizontal ? "h-16 w-[2px]" : "w-16 h-[2px]"} bg-neutral-600 opacity-50 rounded-full`}></div>
        </div>
        <div className="z-10 flex gap-1">
          <IconButton
            ...
            ariaLabel="Collapse"
            onClick={onCollapse}
            className="bg-base-secondary hover:bg-neutral-700 transition-colors duration-200 rounded-full"
          />
          <IconButton
      ...
      <div
        ref={secondRef}
        className={twMerge(secondClassName, "transition-all duration-300 ease-in-out")}
