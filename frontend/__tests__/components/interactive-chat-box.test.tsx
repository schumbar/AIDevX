...
describe("InteractiveChatBox", () => {
  ...
  it("should render with dataset upload enabled by default", () => {
    render(<InteractiveChatBox onSubmit={onSubmitMock} onStop={onStopMock} />);

    const chatBox = screen.getByTestId("interactive-chat-box");
    within(chatBox).getByTestId("chat-input");
    within(chatBox).getByTestId("upload-dataset-input");
  });

  it("should not render dataset upload when disabled", () => {
    render(<InteractiveChatBox onSubmit={onSubmitMock} onStop={onStopMock} enableDatasetUpload={false} />);

    const chatBox = screen.getByTestId("interactive-chat-box");
    within(chatBox).getByTestId("chat-input");
    expect(within(chatBox).queryByTestId("upload-dataset-input")).not.toBeInTheDocument();
  });
  ...
  it("should display the image previews when images are uploaded", async () => {
    ...
    const input = screen.getByTestId("upload-dataset-input");
  ...
  it("should handle dataset uploads correctly", async () => {
    const user = userEvent.setup();
    render(<InteractiveChatBox onSubmit={onSubmitMock} onStop={onStopMock} />);

    const datasetFile = new File(["csv data"], "data.csv", { type: "text/csv" });
    const input = screen.getByTestId("upload-dataset-input");

    await user.upload(input, datasetFile);

    // Dataset files should be displayed as text chips, not image previews
    expect(screen.queryAllByTestId("image-preview")).toHaveLength(0);
    expect(screen.getByText("data.csv")).toBeInTheDocument();

    // Submit the message with dataset
    const textarea = within(screen.getByTestId("chat-input")).getByRole("textbox");
    await user.type(textarea, "Process this dataset");
    await user.keyboard("{Enter}");

    // Verify onSubmit was called with the message and dataset file
    expect(onSubmitMock).toHaveBeenCalledWith("Process this dataset", [datasetFile]);

    // Dataset should be cleared after submission
